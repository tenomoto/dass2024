...
$\mathbf{x}=(1,\,3,\,5)$（青）及び$\mathbf{x}=(1.1,\,3.3,\,5.5)$（橙）から無次元時刻10まで積分した結果の3次元可視化結果を示す。
```{r}
#| code-fold: true
...
実線及び破線は、時間積分にRK4を用いて$\mathbf{x}=(1, 3, 5)$と$(1.1, 3.3, 5.5)$から積分したもので、点線はEuler法で$\mathbf{x}=(1, 3, 5)$から積分したものである。
...
for (i in 1:ns) {
  ...
  lines(t, x.eulp[i,], lwd=2, lty=4, col=tab10.new[i])
  abline(h=off[i], lty=3, col=tab10.new[i])
}
legend("topright", legend=c("RK4", "RK4p", "Euler", "Eulerp"), lty=c(1, 2, 3, 4), ncol=2)
```
